...
class Main(QMainWindow, MainUI):
    def __init__(self, parent=None):
        ...
        self.comboBox.addItem('Color Layout Algorithm Mean Color')
        self.comboBox.addItem('Color Layout Algorithm Histogram')
    ...
    def display_video(self, path):
        finalPath = 'vedios/' + path
        cap = cv.VideoCapture(finalPath)
    ...
    def showResults(self):
        ...
        if selector == 'Mean Color Algorithm':
            ...
            for path in matchedPaths:
                ...
                cv.waitKey(0)
            cv.waitKey(0)
        elif selector == 'Color Layout Algorithm Histogram':
            matchedPaths = []
        ...
        elif selector == 'Color Layout Algorithm Mean Color':
            matchedPaths = []
            queryMeanColor = featureExtraction.get_color_layout2(self.image)
            results = DB.colorLayout_find2()
            for result in results:
                sum = 0
                for i in range(len(queryMeanColor)):
                    modelMeanColor = result['colorLayout2'][i]
                    sum = sum + self.calculate_distance(queryMeanColor[i], modelMeanColor)
                avg = sum / 4
                if avg > 85:
                    matchedPaths.append(result['path'])
            for path in matchedPaths:
    ...
    def show_video_result(self):
        ...
        while self.video.isOpened():
            ...
            if len(keyFrame) == 0:
                ...
                hist.append(featureExtraction.get_histogram(keyFrame))
            else:
                ...
                if compare < 0.7:
                    ...
                    keyFrames.append(keyFrame)
                    meanColor.append(featureExtraction.get_mean(keyFrame))
                    hist.append(frameHist)
            cv.imshow('frame', frame)
        # results = DB.mean_color_find_video()
        results = DB.histogram_find_video()
        for result in results: #Loop of database videos
            match = False
            count = 0
            for keyFrameHist in hist: #loop of meanColor query video
                for modelHist in result['hist']:
                    match = False
                    # sum = self.calculate_distance(modelMeanColor, keyFrameMean)
                    sum = self.compareHist(keyFrameHist, np.float32(modelHist))
                    if sum > 0.6:
                        count = count + 1
                        match = True
                        break
    ...
    def initDB(self):
        ROOT_DIR = os.path.dirname(os.path.abspath("images"))
        ...
        for imgPath in images:
            ...
            #4- Color Layout Algorithm using mean color
            colorLayout2 = featureExtraction.get_color_layout2(img)
            colorLayoutList2 = []
            for quarter in colorLayout2:
                colorLayoutList2.append(quarter)
            record["colorLayout2"] = colorLayout2
            DB.insert(record)
    ...
    def init_video_DB(self):
        ...
        ROOT_DIR = os.path.dirname(os.path.abspath("vedios"))
        # path = "C:/Users/Mohamed Ramadan/PycharmProjects/multimedia/Github/img"  # Add images dataset full path
        path = ROOT_DIR + '/vedios'
        ...
        for vedioPath in vedios:
            ...
            while cap.isOpened():
                ...
                if len(keyFrame) == 0:
                    ...
                    keyFrames.append(keyFrame)
                    meanColor.append(featureExtraction.get_mean(keyFrame))
                    hist.append(featureExtraction.get_histogram(keyFrame).tolist())
                else:
                    frameHist = featureExtraction.get_histogram(frame)
                    keyFrameHist = featureExtraction.get_histogram(keyFrame)
                    compare = self.compareHist(frameHist, keyFrameHist)
